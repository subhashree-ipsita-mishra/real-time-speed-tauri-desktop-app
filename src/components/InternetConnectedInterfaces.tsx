...
import { RefreshCw, Globe, AlertCircle, Loader } from "lucide-react";
...
function InternetConnectedInterfaces() {
  const [interfaces, setInterfaces] = useState<NetworkInterfaceInfo[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  async function fetchInternetConnectedInterfaces() {
    try {
      setLoading(true);
      setError(null);
      const connectedInterfaces: NetworkInterfaceInfo[] = await invoke(
        "get_internet_connected_interfaces"
      );
      setInterfaces(connectedInterfaces);
    } catch (err) {
      setError(
        "Failed to fetch internet connected interfaces: " +
          (err as Error).message
      );
      console.error("Error fetching internet connected interfaces:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    fetchInternetConnectedInterfaces();
  }, []);
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      <div className="bg-gradient-to-r from-purple-500 to-indigo-600 p-4">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={fetchInternetConnectedInterfaces}
            disabled={loading}
            ...
            {interfaces.length === 0 ? (
              <div className="text-center py-6">
                ...
                <h3 className="mt-1 text-base font-medium text-gray-900">
                  No internet connected interfaces found
                </h3>
                <p className="mt-1 text-sm text-gray-500">
                  No network interfaces with internet connectivity were
                  detected.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {interfaces.map((iface, index) => (
                  <div
                    key={index}
                    className="bg-gray-50 rounded-lg p-3 border border-gray-200 hover:border-purple-300 hover:shadow transition-all duration-200"
                  >
            ...
            <button
              ...
              className={`inline-flex items-center px-3 py-1.5 rounded font-medium text-sm ${
                loading
                  ...
                  : "bg-purple-500 hover:bg-purple-600 text-white shadow hover:shadow-md transition-all duration-200"
              }`}
            >
              <RefreshCw
...
export default InternetConnectedInterfaces;
