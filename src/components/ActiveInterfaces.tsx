...
function ActiveInterfaces() {
  ...
  async function fetchActiveNetworkInterfaces() {
    try {
      setLoading(true);
      setError(null);
      const activeNetworkInterfaces: NetworkInterfaceInfo[] = await invoke(
            ...
            {interfaces.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  No active network interfaces were detected on your system.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {interfaces.map((iface, index) => (
                  <div
                    key={index}
                    className="bg-gray-50 rounded-lg p-3 border border-gray-200 hover:border-green-300 hover:shadow transition-all duration-200"
                  >
...
export default ActiveInterfaces;
