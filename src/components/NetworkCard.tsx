...
export default function NetworkCard() {
  ...
  console.log("NetworkCard networkStats:", networkStats);
  console.log("NetworkCard adapters:", adapters);
        ...
        {isTracking && networkStats.length > 0 ? (
          <div className="space-y-2">
            {networkStats.map((stat, index) => {
              ...
              const icon = matchingAdapter ? getAdapterIcon(matchingAdapter.InterfaceType) : 
                           <Network size={16} className="mr-2 text-gray-500" />;
              const type = matchingAdapter ? interfaceTypeToString(matchingAdapter.InterfaceType) : "Unknown";
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      {icon}
                      <div>
                        <span className="font-medium truncate max-w-[150px]">{stat.name}</span>
                        <span className="ml-1 text-xs bg-gray-200 rounded px-1.5 py-0.5">
                          {type}
                        </span>
                      </div>
                    </div>
                    <span className="font-semibold text-gray-700">{formatBytes(stat.value)}</span>
                  </div>
                </div>
              );
            })}
