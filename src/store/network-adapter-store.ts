...
function parseCSV(csvString: string): NetworkAdapter[] {
  ...
  const headers = lines[0].split('","').map(header => {
    return header.replace(/^"|"$/g, ''); // Remove leading and trailing quotes
  });

  return lines.slice(1) // Skip header line
...
export const useNetworkAdapterStore = create<NetworkAdapterStore>()(
  (set) => ({
    adapters: [],
    isLoading: false,
    error: null,
